...
module Tolk
  class Locale < ActiveRecord::Base
    ...
    def phrases_without_translation(page = nil, options = {})
      ...
      options = {:page => page, :per_page => Phrase.per_page}.merge(options)
      result = phrases.page(options[:page]).per(options[:per_page])
    ...
    def search_phrases_without_translation(query, page = nil, options = {})
      ...
      options = {:page => page}.merge(options)
      result = phrases.page(options[:page])
      ActiveRecord::Associations::Preloader.new result, :translations
      result
    end
    ...
    def find_phrases_with_translations(page, conditions = {})
      result = Tolk::Phrase.joins(:translations).
        where(:'tolk_translations.locale_id' => self.id).
